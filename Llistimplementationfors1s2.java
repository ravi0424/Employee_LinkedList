...
public class Llistimplementationfors1s2 {
    public static Llist.Emp_details readEmployeeDetails(Scanner sc) {
        ...
        return new Llist.Emp_details(empId, name, role, exp, age);
    }
    public static void DataStoredinFile(Llist list) throws IOException{
        File f=new File("D:\\LL\\LL/Data.txt");
        if(!f.exists()){
            f.createNewFile();
        }
        String data=list.FinalWrite();
        if(data!=null){
    ...
    public static void main(String[] args) throws IOException {
        ...
        File file= new File("D:\\LL\\LL/Data.txt");
        Scanner sr=new Scanner(file);
        String s=new String();
        while(sr.hasNextLine()){
            String line = sr.nextLine();
            String[] NodeData=line.split(",");
            int id = Integer.parseInt(NodeData[0].trim());
            String name = NodeData[1].trim();
            String role = NodeData[2].trim();
            float exp = Float.parseFloat(NodeData[3].trim());
            int age = Integer.parseInt(NodeData[4].trim());

            Llist.Emp_details emp = new Llist.Emp_details(id, name, role, exp, age);
            list.insertLast(emp);
            }
        sr.close();
        do {
            ...
            switch (choice) {
                case 1:
                    Llist.Emp_details empFirst = readEmployeeDetails(sc);
                    list.insertFirst(empFirst);
                    // scenario 2 at the end of every operation the data in a file is overidded
                    DataStoredinFile(list);
                    break;

                case 2:
                    Llist.Emp_details empLast = readEmployeeDetails(sc);
                ...
                case 3:
                    ...
                    pos = sc.nextInt();
                    sc.nextLine();
                    Llist.Emp_details empAtPos = readEmployeeDetails(sc);
                ...
                case 12:
                    ...
                    break;
                case 13:
                    //  scenario 1 write the linked list objects at the end of the program 
                    // DataStoredinFile(list);
